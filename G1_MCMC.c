...
int pop;
    ...
    //calls = (size_t) (gsl_rng_uniform(r2) * 300 + 1);
	...
	//getc(stdin);
     ...
     // Params.PARS[14] = PosteriorsIn[SEIRParamNum][9];	//gamma2
      ...
      //Params.PARS[19] = PosteriorsIn[SEIRParamNum][11];	//delta1
      // Params.PARS[21] = PosteriorsIn[SEIRParamNum][12];	//delta1

      //Params.PARS[27] = PosteriorsIn[SEIRParamNum][13];	//delta1
     // Params.PARS[28] = PosteriorsIn[SEIRParamNum][14];	//delta1

     // Params.PARS[29] = PosteriorsIn[SEIRParamNum][15];	//delta1
      //Params.PARS[30] = PosteriorsIn[SEIRParamNum][16];
      //Params.PARS[31] = PosteriorsIn[SEIRParamNum][17];	//delta1
      //Params.PARS[32] = PosteriorsIn[SEIRParamNum][18];
       ...
       // printf("PopWiseMean %e, PopeWiseMeanLog = %e, PopWiseM2Log = %e\n", tempwisemean[pop], tempwisemeanlog[pop], tempwisem2log[pop]);
		...
		for(i = 0; i < FERAL_SETS; i++){
			...
			PopWiseMeanLog[pop] += tempwisemeanlog[pop];
       		...
       		//FILE *fp6;
    			//fp6 = fopen("WAIC_D1_components2.dat","a");
...
///I deleted this because life was too short


/////////////////////////////Begin reading in principal component analysis results//////////////////////
	int NumberOfParams=7;			// 32 parameters - alpha1,beta,sigma,rho1,phi,m1,delta1
	...
	double SigmaInflation=2.0;  //MG temporary value //CK// Dave had 4 as his sigma inflation factor
...
//		{
		...
		PCAparams[6] = round(PCAparams[6]);
		Old_Params[6] = round(Old_Params[6]);
		...
		//PCAparams[12] = round(PCAparams[12]);
		//Old_Params[12] = round(Old_Params[12]);
	...
	//Params.PARS[6] = PCAparams[1];//variance
	Params.PARS[7] = PCAparams[1];//rho1
	Params.PARS[8] = PCAparams[2];//phi
	Params.PARS[9] = PCAparams[3];//m1
	//Params.PARS[13] = PCAparams[5];//gamma1
	Params.PARS[18] = PCAparams[4];//delta1
	//Params.PARS[19] = PCAparams[5];//alpha2
	//Params.PARS[27] = PCAparams[6];//m2
	//Params.PARS[29] = PCAparams[7];//rho2
	//Params.PARS[31] = PCAparams[8];//delta2
	Params.PARS[33] = PCAparams[5];//gamma3
	Params.PARS[34] = PCAparams[6];//gamma4
for(i = 0; i < 37; i++){
...
redo = 0;
	...
	//Params.PARS[6] =  Old_Params[1];//variance
	Params.PARS[7] =  Old_Params[1];//rho1
	Params.PARS[8] =  Old_Params[2];//phi
	Params.PARS[9] =  Old_Params[3];//m1
	//Params.PARS[13] = Old_Params[5];//gamma1
	Params.PARS[18] =  Old_Params[4];//delta1
	//Params.PARS[19] = Old_Params[5];//alpha2
	//Params.PARS[27] = Old_Params[6];//m2
	//Params.PARS[29] = Old_Params[7];//rho2
	//Params.PARS[31] = Old_Params[8];//delta2
	Params.PARS[33] = Old_Params[5];//rho2
	Params.PARS[34] = Old_Params[6];//delta2
for(i = 0; i < 37; i++){
	if(i == 4 || i == 7 || i == 8 || i == 13 || i == 18 || i == 19 || i == 21 || i == 29 || i == 30 || i == 31 || i == 32 || i == 33 || i == 35 || i == 23){
		if(Params.PARS[i] < pow(10,Params.parm_low[i])){
			Params.PARS[i] = pow(10,Params.parm_low[i]);
		...
		// ------------------------------------------ output results to file  --------------------------------------- //
			...
			//printf("loopnumber = %d\n", LoopNumber);
 			...
 			fp2 = fopen(testbuff,"a"); //appending to the end of the file
  	 		 ...
  	 		 fprintf(fp2,"%d, %d, %d, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f  \n",num_runs, calls, LoopNumber, error, Params.PARS[4], Params.PARS[5], Params.PARS[6],  Params.PARS[7],
			 Params.PARS[8],
			 Params.PARS[9],
			 Params.PARS[11],
			 Params.PARS[12],
